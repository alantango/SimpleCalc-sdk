...
This compiles the client into a jar located at `./target/simple-calc-sdk-1.0.0.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'my-apig-api-examples:simple-calc-sdk:1.0.0'
}
```
...
This will compile the client and package all dependencies into a jar located at `./target/simple-calc-sdk-1.0.0.jar`.
...
```java
SimpleCalcSdk client = SimpleCalcSdk.builder()
...
```java
SimpleCalcSdk client = SimpleCalcSdk.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`SimpleCalcSdk`) is created for all actions (resource + method) in your API.
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of SimpleCalcSdkException.
```java
try {
    client.getABOp(...);
} catch(SimpleCalcSdkException e) {
   // All service exceptions will extend from SimpleCalcSdkException.
   // Any unknown or unmodeled service exceptions will be represented as a SimpleCalcSdkException.
